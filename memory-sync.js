...
export class MemorySync {
  ...
  async syncActiveProjectToMemory(projectId) {
    try {
      ...
      const memoryContext = {
        ...
        next_logical_areas: this.identifyNextLogicalAreas(
          htaData,
          learningHistory,
        ),
        today_progress: todaySchedule.blocks
          ? todaySchedule.blocks.filter((b) => b.completed).length +
            "/" +
            todaySchedule.blocks.length +
            " blocks completed"
          : "No schedule today",
        suggested_memory_queries: this.generateMemoryQueries(
          config,
          learningHistory,
        ),
      ...
      await this.dataPersistence.logError("syncActiveProjectToMemory", error, {
        projectId,
      });
      return {
        project_id: projectId,
  ...
  generateProgressSummary(learningHistory, htaData) {
    ...
    const completedBranches =
      htaData.strategicBranches?.filter((b) => b.completed)?.length || 0;
  ...
  analyzeCompletionPatterns(learningHistory) {
    ...
    const breakthroughRate =
      recentCompletions.filter((c) => c.breakthrough).length /
      recentCompletions.length;
  ...
  identifyNextLogicalAreas(htaData, learningHistory) {
    const completedTopics =
      learningHistory.completedTopics?.map((c) => c.topic) || [];
    ...
    return (
      readyNodes.slice(0, 3).map((node) => node.title) || [
  ...
  generateMemoryQueries(config, learningHistory) {
    ...
    const recentTopics =
      learningHistory.completedTopics?.slice(-3).map((c) => c.topic) || [];

    if (goal) {
      queries.push(`What have I learned recently about ${goal}?`);
      queries.push(
        `What questions or challenges have emerged in my ${goal} journey?`,
      );
    }
